...
func TestCmdsServer(t *testing.T) {
	...
	info := func(t *testing.T) {
		...
		expected := &Server{
			...
			DownloadQuota:                          18446744073709551615,
			UploadQuota:                            18446744073709551615,
			ClientConnections:                      2556,
			QueryClientConnections:                 84223,
			TotalBytesDownloaded:                   17282863873,
			TotalBytesUploaded:                     8719954284,
			TotalPacketLossSpeech:                  0.0000,
			TotalPacketLossKeepalive:               0.0000,
			TotalPacketLossControl:                 0.0000,
			TotalPacketLossTotal:                   0.0000,
			TotalPing:                              0.0000,
			MinAndroidVersion:                      0,
			MiniOSVersion:                          0,
			FileTransferBytesSentTotal:             1119299288,
			FileTransferBytesReceivedTotal:         383216474,
			PacketsSentSpeech:                      323798306,
			BytesSentSpeech:                        44239747535,
			PacketsReceivedSpeech:                  176899777,
			BytesReceivedSpeech:                    24384042693,
			PacketsSentKeepalive:                   55749607,
			BytesSentKeepalive:                     2285733887,
			PacketsReceivedKeepalive:               55668655,
			BytesReceivedKeepalive:                 2338194933,
			PacketsSentControl:                     2392370,
			BytesSentControl:                       529512085,
			PacketsReceivedControl:                 2392484,
			BytesReceivedControl:                   228820306,
			PacketsSentTotal:                       381940283,
			BytesSentTotal:                         47054993507,
			PacketsReceivedTotal:                   234960916,
			BytesReceivedTotal:                     26951057932,
		}
	...
	instanceinfo := func(t *testing.T) {
		...
		expected := &Instance{
			...
			ServerQueryMaxConnectionsPerIP: 5,
		}
